...
function editarPresentacion($id, $titulo, $descripcion){

    $bdConexion = ConexionBD::obtenerInstancia();
    $conexion = $bdConexion->getConnection();
    $respuesta = Presentacion::actualizarPresentacion($conexion, $id, $titulo, $descripcion);
    $conexion = null;
    return $respuesta;
}
...
function procesarFormulario() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $nuevoOrden = $_POST['ordenNuevoDiapositivas'];
        $errores = [];
        ...
        if(count($errores) > 0){
            ...
            $_SESSION['confirmacion'] = editarPresentacion($id, $titulo, $descripcion);
